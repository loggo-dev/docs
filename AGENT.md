...
This site documents [Loggo](https://github.com/loggo-dev/loggo), a self-hosted note app — this repo (`loggo-dev/docs`) only holds the documentation, not the application source. For anything not covered here — implementation details, source code, open issues, exact env var handling — fetch the app repo directly rather than guessing.
